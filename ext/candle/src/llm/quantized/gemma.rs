...
impl QuantizedGemma {
    ...
    /// Load a quantized Gemma model from HuggingFace Hub
    ...
    async fn download_tokenizer(api: &Api, repo: &ApiRepo, model_id: &str) -> CandleResult<std::path::PathBuf> {
        ...
        if model_id.starts_with("TheBloke/") {
            ...
            // e.g., "TheBloke/gemma-2b-it-GGUF" -> "google/gemma-2b-it"
...
impl TextGenerator for QuantizedGemma {
    fn generate(
        &mut self,
        prompt: &str,
        config: &GenerationConfig,
