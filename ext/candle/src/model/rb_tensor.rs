...
use crate::model::utils::{actual_dim, actual_index};
...
impl RbTensor {
  ...
  pub fn index_select(&self, rhs: &Self, dim: i64) -> RbResult<Self> {
      ...
      Ok(RbTensor(self.0.index_select(rhs, dim).map_err(wrap_candle_err)?))
  }
  ...
  pub fn broadcast_add(&self, rhs: &Self) -> RbResult<Self> {
      Ok(RbTensor(self.0.broadcast_add(rhs).map_err(wrap_candle_err)?))
  }
  ...
  pub fn broadcast_sub(&self, rhs: &Self) -> RbResult<Self> {
      Ok(RbTensor(self.0.broadcast_sub(rhs).map_err(wrap_candle_err)?))
  }
  ...
  pub fn broadcast_mul(&self, rhs: &Self) -> RbResult<Self> {
      Ok(RbTensor(self.0.broadcast_mul(rhs).map_err(wrap_candle_err)?))
  }
  ...
  pub fn broadcast_div(&self, rhs: &Self) -> RbResult<Self> {
      Ok(RbTensor(self.0.broadcast_div(rhs).map_err(wrap_candle_err)?))
  }
  ...
  pub fn where_cond(&self, on_true: &Self, on_false: &Self) -> RbResult<Self> {
      Ok(RbTensor(
          self.0.where_cond(on_true, on_false).map_err(wrap_candle_err)?,
      ))
  }
  ...
  pub fn broadcast_as(&self, shape: Vec<usize>) -> RbResult<Self> {
      Ok(RbTensor(self.0.broadcast_as(shape).map_err(wrap_candle_err)?))
  }
  ...
  pub fn broadcast_left(&self, shape: Vec<usize>) -> RbResult<Self> {
      Ok(RbTensor(self.0.broadcast_left(shape).map_err(wrap_candle_err)?))
  }
  ...
  pub fn transpose(&self, dim1: usize, dim2: usize) -> RbResult<Self> {
      Ok(RbTensor(self.0.transpose(dim1, dim2).map_err(wrap_candle_err)?))
  }
  ...
  pub fn narrow(&self, dim: i64, start: i64, len: usize) -> RbResult<Self> {
      ...
      Ok(RbTensor(self.0.narrow(dim, start, len).map_err(wrap_candle_err)?))
  }
  ...
  pub fn argmax_keepdim(&self, dim: i64) -> RbResult<Self> {
      let dim = actual_dim(self, dim).map_err(wrap_candle_err)?;
      Ok(RbTensor(self.0.argmax_keepdim(dim).map_err(wrap_candle_err)?))
  }
  ...
  pub fn argmin_keepdim(&self, dim: i64) -> RbResult<Self> {
      let dim = actual_dim(self, dim).map_err(wrap_candle_err)?;
      Ok(RbTensor(self.0.argmin_keepdim(dim).map_err(wrap_candle_err)?))
  }
  ...
  pub fn to_device(&self, device: RbDevice) -> RbResult<Self> {
      ...
      Ok(RbTensor(self.0.to_device(&device).map_err(wrap_candle_err)?))
  }
}
